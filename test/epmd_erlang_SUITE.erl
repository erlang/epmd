...
-define(CONN_SLEEP, 50).        % Inital, doubles every retry
...
init_per_testcase(Test, Config) ->
    ok = application:unset_env(epmd, relaxed_command_check),
    case Test of
        unregister_others_name_1 ->
            ok = application:set_env(epmd, relaxed_command_check, true);
        unregister_others_name_2 ->
            ok = application:set_env(epmd, relaxed_command_check, true);
        socket_reset_before_alive2_reply_is_written ->
            ok = application:set_env(epmd, relaxed_command_check, true),
            ok = application:set_env(epmd, delay_write, 1);
        _ ->
            ok
    end,
    ok = application:set_env(epmd, port, ?PORT),
...
end_per_testcase(Test, _Config) ->
    case Test of
        ...
        socket_reset_before_alive2_reply_is_written ->
            ok = application:unset_env(epmd, relaxed_command_check),
            ok = application:unset_env(epmd, delay_write);
        _ ->
            ok
    end,
    ok = application:stop(epmd),
    ok = application:unset_env(epmd, port),
    ok.
...
-define(u32(X1,X2,X3,X4), (((X1) bsl 24) bor ((X2) bsl 16) bor ((X3) bsl 8) bor X4)).
...
register_many(I, N, Prefix) ->
    ...
    case register_node(Name, Port) of
        ...
        Any ->
            io:format("Can't register: ~w of 1..~w ~w~n", [Name,N,Any]),
            []
    end.
...
unregister_many([{Name,_Port,Sock} | Socks]) ->
    case close(Sock) of
        ...
        Any ->
            io:format("Can't unregister: ~w reason ~w~n", [Name,Any]),
            error
    end.
...
alive_req_too_small_2(Config) when is_list(Config) ->
    {ok,Sock} = connect(),
    M  = [?EPMD_ALIVE2_REQ, put16(?DUMMY_PORT),$M,0, put16(5), put16(5)],
    ok = send(Sock, [size16(M), M]),
    sleep(?MEDIUM_PAUSE),
    closed = recv(Sock,1),
    ok.
...
no_live_killing(Config) when is_list(Config) ->
    ...
    % don't kill this node
    % rethink erlang:halt/0
    % sleep(?MEDIUM_PAUSE),
    % {ok,Sock3} = connect(),
    % M3 = [?EPMD_KILL_REQ],
...
flusher() ->
    receive
        {_,{data,Msg}} ->
            io:format("from epmd: ~s~n", [Msg]),
            flusher();
        Msg ->
            io:format("from epmd (unexpected): ~p~n", [Msg]),
            flusher()
    end.
...
%% Retry after 2 seconds, to avoid TIME_WAIT socket exhaust.
connect_sturdy() ->
    connect("localhost",?PORT, passive, 2000, 3).
...
connect(Addr, Port, Mode, Sleep, Retry) ->
    case connect_repeat(Addr, Retry, Port, Mode, Sleep) of
        ...
        {error,Reason} ->
            ...
            error;
        Any ->
            io:format("connect: unknown message: ~w~n",[Any]),
            exit(1)
    end.
...
connect_repeat(Addr,Retry, Port, Mode, Sleep) ->
    case connect_mode(Addr,Port, Mode) of
        {ok,Sock} ->
            {ok,Sock};
        {error,Reason} ->
            io:format("connect: error: ~w sleeping for ~w~n",[Reason,Sleep]),
            timer:sleep(Sleep),
            connect_repeat(Addr, Retry - 1, Port, Mode, 2*Sleep);
        Any ->
            io:format("connect: unknown message: ~w~n",[Any]),
            exit(1)
    end.
        ...
        {ok,Sock} ->
            case send(Sock, [size16(Req), Req]) of
                ok ->
                    {ok,Sock};
                Other ->
                    io:format("Failed to send ~w on sock ~w: ~w~n",
                              [Req,Sock,Other]),
                    error
            end;
        Other ->
            io:format("Connect failed when sending ~w: ~p~n",
                      [Req, Other]),
            error
    end.
